...
export default function StudentPromotion() {
  ...
  // Select helpers
  const toggleSelLeft = (id) =>
    setSelLeft((prev) => {
      const n = new Set(prev);
      n.has(id) ? n.delete(id) : n.add(id);
      return n;
    });
  const toggleSelRight = (id) =>
    setSelRight((prev) => {
      const n = new Set(prev);
      n.has(id) ? n.delete(id) : n.add(id);
      return n;
  ...
  const preview = async () => {
    if (!bodyBase.from_class_id || !bodyBase.to_class_id)
      return toast.error("Pick both current and next classes.");
    if (!bodyBase.student_ids.length)
      return toast.error("Move some students to the next list.");
    try {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
        ...
        <div className="p-3 border rounded-lg">
          ...
          <select
            ...
            value={fromYear?.id || ""}
            onChange={(e) =>
              setFromYear(years.find((y) => String(y.id) === e.target.value))
            }
          >
        ...
        <div className="p-3 border rounded-lg">
          ...
          >
            <option value="">Select year</option>
            {[...years.map((y) => Number(y.year)), Number(fromYear?.year) + 1]
              ...
              <tbody>
                {leftFiltered.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td>
                      {/* current class name not needed; video shows; omit to keep simple */}
                    </td>
                    <td>{s.section}</td>
                  </tr>
                ))}
              ...
              <tbody>
                {right.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td className="whitespace-nowrap">{s.name}</td>
                    <td>
                      {toClasses.find((c) => String(c.id) === String(toClassId))
                        ?.name || "-"}
                    </td>
                    <td>
                      {toSectionId
                        ? toSections.find(
                            (sec) => String(sec.id) === String(toSectionId)
                          )?.name
                        : "Not selected"}
                    </td>
                  </tr>
