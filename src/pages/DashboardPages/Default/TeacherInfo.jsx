...
//   const anyLinkedInFiltered = useMemo(
//     () => filteredTeachers.some((t) => isLinked(t)),
//     [filteredTeachers]
//   );
...
//         ...prev,
//         username: generated,
//         // auto-password = same as username if not already set
//         password: prev.password || generated,
//       }));
...
//         // auto-password = username when empty
//         const finalPassword = userForm.password || finalUsername;

//         // guard against account-level email/phone duplicates
//         const emailTaken = await valueUsedByUser(
//           "email",
//           userForm.email || form.contact_email
//         );
//         const phoneTaken = await valueUsedByUser(
//           "phone",
//           userForm.phone || form.contact_phone
//         );
...
//           role: "Teacher",
//           password: finalPassword, // ALWAYS send something (username if empty)
...
//         toast.success(
//           `Login "${finalUsername}" created & linked. Password: ${finalPassword}`
//         );
//       }
...
//     // Only include teachers that have linked login
...
//         "User ID": t.user_username || "",
//         Pass: t.user_username || "", // password = username convention
//       }));

//     if (!rows.length) {
//       toast.error("No teachers with login to export.");
//       return;
//     }
...
//     const filename = `teachers_credentials_${now.getFullYear()}-${pad(
//       now.getMonth() + 1
...
//               disabled={!anyLinkedInFiltered}
//               className="px-3 py-2 rounded-xl border text-sm bg-emerald-50 text-emerald-700 hover:bg-emerald-100 disabled:opacity-50"
//             >
//               Download Excel
//             </button>
...
//               <th className="px-3 py-2 text-left font-semibold">Pass</th>
...
//                   <td className="px-3 py-2">{isLinked(t) ? t.user_username || "-" : "-"}</td>
//                   <td className="px-3 py-2">
//                     {isLinked(t) ? t.user_username || "-" : "-"}
//                   </td>
//                   <td className="px-3 py-2">
//                     {isLinked(t) ? (
...
//                       // prefill username & password only if empty
//                       if (!userForm.username && !userForm.password && createLogin) {
//                         const suggested = suggestUsername(v);
//                         setUserForm((prev) => ({
//                           ...prev,
//                           username: suggested,
//                           password: suggested,
...
//                         if (checked && form.full_name && !userForm.username && !userForm.password) {
//                           const suggested = suggestUsername(form.full_name);
//                           setUserForm((prev) => ({
//                             ...prev,
//                             username: suggested,
//                             password: suggested,
...
//                           Leave blank to auto-suggest from first name (e.g., karim345). Password
//                           will be the same as username, unless you type your own.
//                         </p>
//                       </div>
...
//                       <div>
//                         <label className="block text-xs font-medium text-slate-600 mb-1">
//                           Password (auto = username)
//                         </label>
//                         <input
//                           type="text"
//                           className="w-full border border-slate-300 p-2 rounded-lg"
//                           placeholder="Leave empty to use the username"
//                           value={userForm.password}
//                           onChange={(e) => onUserChange("password", e.target.value)}
//                         />
//                         <p className="text-[11px] text-slate-500 mt-1">
//                           If empty, password will be the same as the username.
//                         </p>
//                       </div>
...
//                             onChange={(e) =>
//                               onUserChange("must_change_password", e.target.checked)
//                             }
//                           />
...
//----------------------------------------------
// this will generate different username and password
// src/pages/DashboardPages/academics/TeacherInfo.jsx
import { useEffect, useMemo, useState, useRef } from "react";
import { useNavigate } from "react-router-dom";
import Select from "react-select";
...
/* ---------------- Password helpers (random, not username) -------------- */

const PASSWORD_STORAGE_KEY = "teacher_plain_passwords_v1";
...
// simple random password: 10 chars letters+digits+symbols
...
export default function TeacherInfo() {
  ...
  // cached plain passwords per user_id (localStorage)
  ...
  useEffect(() => {
    (async () => {
      // load cached passwords first
      setSavedPasswords(loadPasswordMap());
      await loadSubjects();
      await loadTeachers();
    })();
  }, []);
  ...
  const teachers = useMemo(
    () =>
      (rawTeachers || []).map((t) => {
        ...
        const userId = t.user_id ?? t.user?.id ?? null;
        const plainPassword = userId ? savedPasswords[String(userId)] || null : null;
        ...
        return {
          ...
          user: t.user ?? null,
          user_id: userId,
          user_username: t.user_username ?? t.user?.username ?? null,
          user_plain_password: plainPassword,
        };
      }),
    [rawTeachers, subjectLabelById, savedPasswords]
  );
  ...
  const checkAndFixUsername = async () => {
    ...
    if (!u) {
      ...
      setUserForm((prev) => ({
        ...prev,
        username: generated,
        // if no password typed yet, also generate a random password
  ...
  const checkEmailUnique = async () => {
    ...
    if (mySeq !== emailCheckSeq.current) return true;
  ...
  const checkPhoneUnique = async () => {
    ...
    if (mySeq !== phoneCheckSeq.current) return true;
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (form.contact_phone) fd.append("contact_phone", form.contact_phone);
      if (form.profile) fd.append("profile", form.profile);
      if (form.subject) fd.append("subject", String(form.subject));
      if (form.photo) fd.append("photo", form.photo);
      ...
      if (createLogin && teacherId) {
        ...
        // password: use typed one OR generated random
        let finalPassword = (userForm.password || "").trim();
        if (!finalPassword) {
        ...
        // cache plain password locally for this user id
        if (newUser?.id) {
          setSavedPasswords((prev) => {
            const next = { ...prev, [String(newUser.id)]: finalPassword };
  ...
  const handleDownloadExcel = () => {
    const rows = filteredTeachers
      ...
      .map((t, index) => ({
        ...
        Phone: t.contact_phone || "",
        "User ID": t.user_username || "",
        Pass: t.user_plain_password || t.user_username || "",
      }));
            ...
            {loading ? (
              ...
              filteredTeachers.map((t, i) => (
                <tr key={t.id} className="border-t hover:bg-slate-50/60">
                  ...
                  <td className="px-3 py-2">
                    {isLinked(t)
                      ? t.user_plain_password || t.user_username || "-"
                      : "-"}
                  </td>
                  <td className="px-3 py-2">
                    {isLinked(t) ? (
              ...
              <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const v = e.target.value;
                      setForm((f) => ({ ...f, full_name: v }));
                      // prefill username & random password only if both empty
                      if (!userForm.username && !userForm.password && createLogin) {
                        const suggestedUser = suggestUsername(v);
                ...
                <div>
                  ...
                  <Select
                    ...
                    placeholder="Select subject"
                    value={
                      subjectOptions.find((o) => String(o.value) === String(form.subject)) ||
                      null
                    }
                    onChange={(opt) => setForm((f) => ({ ...f, subject: opt ? opt.value : "" }))}
                    options={subjectOptions}
                  ...
                  <label className="inline-flex items-center gap-2 text-sm font-semibold mb-2">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setCreateLogin(checked);
                        if (checked && form.full_name && !userForm.username && !userForm.password) {
                          const suggestedUser = suggestUsername(form.full_name);
                          const randomPass = generateRandomPassword();
                          setUserForm((prev) => ({
                            ...prev,
                            username: suggestedUser,
                  ...
                  {createLogin && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        />
                        <p className="text-[11px] text-slate-500 mt-1">
                          If empty, a random strong password will be generated.
                        </p>
                      </div>
